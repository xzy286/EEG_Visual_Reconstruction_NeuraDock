...
vlmodel, preprocess_train, feature_extractor = open_clip.create_model_and_transforms(
    model_type, pretrained=r'D:\EEG_Image_decode-main\EEG_Image_decode-main\clip_laion2b_s32b_b79k\open_clip_model.safetensors', precision='fp32', device = device)
...
class EEGDataset():
    ...
    """
    def __init__(self, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 250], classes = None, pictures = None, val_size=None,data_path_list=['preprocessed_eeg_training_selected__300.npy','preprocessed_eeg_test_selected_300.npy']):
        ...
        self.data = self.data[..., time_window[0]:time_window[1]]
        print(self.data.shape)
    ...
    def load_data(self):
        ...
        for subject in self.subjects:
            if self.train:
                ...
                file_name = self.data_path_list[0]
                print(file_name)
                ...
                if subject == self.exclude_subject or self.exclude_subject==None:  
                    file_name = self.data_path_list[1]
                    print(file_name)
                    file_path = os.path.join(self.data_path, subject, file_name)
                    data = np.load(file_path, allow_pickle=True)
        ...
        if self.train:
            ...
            label_tensor = label_tensor.repeat_interleave(2)
            if self.classes is not None:
    ...
    def __getitem__(self, index):
        ...
        if self.pictures is None:
            if self.classes is None:
                index_n_sub_train = self.n_cls * 10 * 2
                index_n_sub_test = self.n_cls * 1 * 40
            else:
                index_n_sub_test = len(self.classes)* 1 * 40
                index_n_sub_train = len(self.classes)* 10 * 2
            # text_index: classes
            if self.train:
                text_index = (index % index_n_sub_train) // (10 * 2)
            else:
                text_index = (index % index_n_sub_test)
            # img_index: classes * 10
            if self.train:
                img_index = (index % index_n_sub_train) // (2)
            else:
                img_index = (index % index_n_sub_test)
        else:
            if self.classes is None:
                index_n_sub_train = self.n_cls * 1 * 2
                index_n_sub_test = self.n_cls * 1 * 40
            else:
                index_n_sub_test = len(self.classes)* 1 * 40
                index_n_sub_train = len(self.classes)* 1 * 2
            # text_index: classes
            if self.train:
                text_index = (index % index_n_sub_train) // (1 * 2)
            else:
                text_index = (index % index_n_sub_test)
            # img_index: classes * 10
            if self.train:
