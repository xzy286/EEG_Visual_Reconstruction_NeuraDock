...
import tqdm
from eegdatasets_leaveone_neuradock import EEGDataset
...
class ATMS(nn.Module):    
    def __init__(self, num_channels=7, sequence_length=250, num_subjects=1, num_features=64, num_latents=1024, num_blocks=1):
...
def main(time_w = [0,250]):
    # Use argparse to parse the command-line arguments
    parser = argparse.ArgumentParser(description='EEG Transformer Training Script')
    parser.add_argument('--data_path', type=str, default="D:\\EEG_Image_decode-main\\EEG_Image_decode-main\\neuradock_data", help='Path to the EEG dataset')
    ...
    parser.add_argument('--subjects', nargs='+', default=["sub-001-preprocessed"], help='List of subject IDs (default: sub-01 to sub-10)')    
    parser.add_argument('--train_file', type=str, default='preprocessed_eeg_training_selected__300.npy')   
    parser.add_argument('--test_file', type=str,  default='preprocessed_eeg_test_selected_300.npy')   
    ...
    for sub in subjects:
        ...
        # eeg_model.load_state_dict(torch.load(r"D:\EEG_Image_decode-main\EEG_Image_decode-main\Generation\models\contrast\ATMS\sub-08\12-11_15-18\40.pth"))
        ...
        if args.insubject:
            train_dataset = EEGDataset(args.data_path, subjects=[sub],time_window=time_w, train=True,data_path_list=[args.train_file,args.test_file])
            test_dataset = EEGDataset(args.data_path, subjects=[sub],time_window=time_w, train=False,data_path_list=[args.train_file,args.test_file])
        else:
            train_dataset = EEGDataset(args.data_path, exclude_subject=sub, subjects=subjects,time_window=time_w, train=True,data_path_list=[args.train_file,args.test_file])
            test_dataset = EEGDataset(args.data_path, exclude_subject=sub, subjects=subjects,time_window=time_w, train=False,data_path_list=[args.train_file,args.test_file])
        ...
        if args.insubject:
            results_file = f"{results_dir}/{args.encoder_type}_{sub}_delay_{str(time_w[0])+"__"+str(time_w[1])}.csv"
        else:
            results_file = f"{results_dir}/{args.encoder_type}_cross_exclude_{sub}_delay_{str(time_w[0])+"__"+str(time_w[1])}.csv"
